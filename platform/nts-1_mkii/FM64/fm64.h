...
#ifdef __float_math_h
  #define POW2(f) fastpow2f(f)
#else
  #include "fastpow.h"
  #define POW2(f) fastpow2f(f)
#endif
...
custom_data("Yamaha DX7-series voice banks", 4096, BANK_COUNT)
dx_voice_t dx_voices[BANK_COUNT][BANK_SIZE] = {};

#ifdef CUSTOM_ALGORITHM_COUNT
custom_data("Korg opsix user algorithm modulation matrices", 42, CUSTOM_ALGORITHM_COUNT)
