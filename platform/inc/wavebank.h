...
#include "osc_api.h"
...
#ifdef FORMAT_PCM12
  #define FORMAT_NAME "12-bit linear PCM"
...
#endif
#ifdef FORMAT_PCM8
  #define FORMAT_NAME "8-bit linear PCM"
...
#endif
#ifdef FORMAT_PCM16
  #define FORMAT_NAME "16-bit linear PCM"
...
#endif
#ifdef FORMAT_PCM32
  #define FORMAT_NAME "32-bit linear PCM"
  ...
  #define FORMAT_NAME "Single precision floating point"
...
static custom_data("Raw " FORMAT_NAME " waveform", SAMPLE_COUNT_TOTAL, WAVE_COUNT)
#endif
#endif
#ifdef WAVEBANK
...
float osc_wavebank(float x, uint32_t idx) {
  ...
  const DATA_TYPE *wt = &wavebank[idx * DATA_TYPE_COUNT];
...
: \
  ...
  return linintf(x0f - (uint32_t)x0f, to_f32(a0), to_f32(a1));
#else
...
: [x0] "r" (x0), [wt0] "r" (&wavebank[(uint32_t)idx * DATA_TYPE_COUNT]), [wt1] "r" (&wavebank[((uint32_t)idx + 1) * DATA_TYPE_COUNT]) \
: \
  );
...
"lsl %[result], %[result], #1\n" \
: [result] "=r" (result) \
: [x] "r" (x), [wt] "r" (&wavebank[idx * DATA_TYPE_COUNT]), [frlsb] "i" (31 - SAMPLE_COUNT_EXP - 15), [xlsb] "i" (31 - SAMPLE_COUNT_EXP), [xwidth] "i" (SAMPLE_COUNT_EXP) \
: "r0" \
  );
  return result;
#else
...
"lsl %[result], %[result], #1\n" \
: [result] "=r" (result) \
: [x] "r" (x), [wt] "r" (&wavebank[idx * DATA_TYPE_COUNT]), [frlsb] "i" (31 - SAMPLE_COUNT_EXP - 15), [xlsb] "i" (31 - SAMPLE_COUNT_EXP), [xwidth] "i" (SAMPLE_COUNT_EXP), [width_recip] "r" (width_recip), [width] "r" (width) \
: "r0" \
  );
  return result;
}
...
q31_t osc_wavebank(q31_t x, q31_t idx) {
  ...
  uint32_t x1 = NEXT_SAMPLE(x0);
  const q31_t fr = (x << SAMPLE_COUNT_EXP) & 0x7FFFFFFF;
  const DATA_TYPE *wt = &wavebank[q31mul(idx, (WAVE_COUNT - 1)) * DATA_TYPE_COUNT];
#ifdef FORMAT_PCM12
  q31_t a0, a1;
  __asm__ volatile ( \
"tst %[x0], #0x1\n" \
...
: [wt0] "r" (wt), [wt1] "r" (wt + DATA_TYPE_COUNT) \
: \
  );
...
void osc_wavebank_preload(uint32_t idx, uint32_t wavenum) {
  ...
  if (wavenum < 17) {
    ...
    for (uint32_t i = 0; i < k_wt_sine_size; i++) {
      ...
      wavebank[idx * DATA_TYPE_COUNT + i] = from_f32(valf1 * sign);
      wavebank[idx * DATA_TYPE_COUNT + i + k_wt_sine_size] = (wavenum < 2 || wavenum == 8 || wavenum > 10) ? from_f32(-valf2) : (DATA_TYPE)0;
...
//todo: stretched DX11/TX81Z waveforms
      for (uint32_t j = 0; j < SAMPLE_COUNT / (k_wt_sine_size * 2); j++) {
        wavebank[idx * DATA_TYPE_COUNT + i * (SAMPLE_COUNT / (k_wt_sine_size * 2)) + j] = from_f32(linintf(delta * j, waves[i], waves[i + 1]));
        wavebank[idx * DATA_TYPE_COUNT + i * (SAMPLE_COUNT / (k_wt_sine_size * 2)) + j + k_wt_sine_size] = from_f32(-linintf(delta * j, waves[k_wt_sine_size - i], waves[k_wt_sine_size - i - 1]));
      }
#else
//todo: squashed waveforms
    ...
    for (uint32_t i = 0; i < k_wt_saw_size; i++) {
      ...
      wavebank[idx * DATA_TYPE_COUNT + i] = from_f32(waves[wavenum * k_wt_saw_lut_size + i]);
      wavebank[idx * DATA_TYPE_COUNT + i + k_wt_saw_size] = from_f32(sign * waves[wavenum * k_wt_saw_lut_size + k_wt_saw_size - i]);
      ...
      for (uint32_t j = 0; j < SAMPLE_COUNT / (k_wt_saw_size * 2); j++) {
        wavebank[idx * DATA_TYPE_COUNT + i * (SAMPLE_COUNT / (k_wt_saw_size * 2)) + j] = from_f32(linintf(delta * j, waves[wavenum * k_wt_saw_lut_size + i], waves[wavenum * k_wt_saw_lut_size + i + 1]));
        wavebank[idx * DATA_TYPE_COUNT + i * (SAMPLE_COUNT / (k_wt_saw_size * 2)) + j + k_wt_saw_size] = from_f32(sign * linintf(delta * j, waves[wavenum * k_wt_saw_lut_size + k_wt_saw_size - i], waves[wavenum * k_wt_saw_lut_size + k_wt_saw_size - i - 1]));
      }
#else
//todo: squashed waveforms
#endif
    ...
    for (uint32_t i = 0; i < k_waves_size; i++) {
      ...
      wavebank[idx * DATA_TYPE_COUNT + i] = from_f32(waves[i]);
#elif SAMPLE_COUNT > k_waves_size
      float delta = (float)k_waves_size / SAMPLE_COUNT;
      for (uint32_t j = 0; j < SAMPLE_COUNT / k_waves_size; j++) {
        wavebank[idx * DATA_TYPE_COUNT + i * (SAMPLE_COUNT / k_waves_size) + j] = from_f32(linintf(delta * j, waves[i], waves[i + 1]));
      }
#else
//todo: squashed waveforms
#endif
